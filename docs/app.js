/* app.js - frontend-only CPU Scheduler Simulator (updated Gantt scaling & responsiveness)
...
function runScheduler(processesInput, config){
  ...
  const selectCandidate = (queue, algo) => {
  ...
  while (completed < n){
    if (queue.length === 0){
      ...
      if (currentTime < nextArr){
        timeline.push({start: currentTime, end: nextArr, pid: null, color: '#111827'});
    ...
    const candidate = selectCandidate(queue, algo);
    if (!candidate) break;
...
/* ===========================
   ...
   (UPDATED: robust scaling, viewBox, resize)
   =========================== */
const Renderer = {
  renderProcessList: () => {
  ...
  drawGantt: () => {
    ...
    const timeline = Array.isArray(State.simulation.timeline) ? State.simulation.timeline.slice() : [];
    // sanitize timeline: ensure numeric starts/ends
    timeline.forEach(t => { t.start = Number(t.start); t.end = Number(t.end); });

    // sort timeline by start to avoid weird ordering
    timeline.sort((a,b) => a.start - b.start);

    const totalTime = Math.max(1, Number(State.simulation.totalTime || (timeline.length ? timeline[timeline.length-1].end : 0)));

    // Determine full pixel width to represent timeline (pxPerUnit * totalTime)
    const pxPerUnit = 80; // px per time unit — increase for better visibility of small durations
    const minWidth = UI.ganttWrapper.clientWidth || 800;
    const totalWidth = Math.max(minWidth, Math.ceil(totalTime * pxPerUnit) + 120); // buffer

    // Prepare SVG
    const height = Math.max(120, UI.ganttWrapper.clientHeight || 160);
    svg.setAttribute('width', totalWidth);
    svg.setAttribute('height', height);
    // viewBox allows proper scaling and scrolling; preserveAspectRatio none so it stretches horizontally
    ...
    const scale = totalWidth / totalTime;

    // Draw blocks
    timeline.forEach(block => {
      // skip invalid blocks
      if (Number.isNaN(block.start) || Number.isNaN(block.end) || block.end <= block.start) return;
      const rectWidth = (block.end - block.start) * scale;
      const x = block.start * scale;
      const y = 16;
      const h = Math.max(28, height - 48);
      ...
      // attach data attributes for tooltip/modal
      rect.setAttribute('data-popup','true');
      rect.setAttribute('data-pid', block.pid===null ? 'idle' : String(block.pid));
      rect.setAttribute('data-start', String(block.start));
      rect.setAttribute('data-end', String(block.end));

      g.appendChild(rect);

      // label (only when wide enough)
      if (rectWidth > 40 && block.pid !== null) {
    ...
    // axis ticks (use ceil(totalTime))
    const tickCount = Math.ceil(totalTime);
    for (let i = 0; i <= tickCount; i++){
      ...
      t.setAttribute('x', x + 2); t.setAttribute('y', height - 2);
      t.setAttribute('fill','#9ca3af'); t.setAttribute('font-size','10'); t.setAttribute('font-family','monospace');
      t.textContent = i;
      svg.appendChild(t);
    }

    // Reattach events for tooltip and modal
    setTimeout(() => {
      svg.querySelectorAll('rect[data-popup="true"]').forEach(r=>{
        ...
        r.addEventListener('click', e=>{
          ...
          showPopup({ title: pid==='idle' ? 'Idle' : `Process ${pid}`, body: `<strong>Start:</strong> ${start}<br><strong>End:</strong> ${end}<br><strong>Duration:</strong> ${end-start}`, buttons:[{text:'Close', cls:'ghost'}] });
        });
      });
    }, 0);
  },
  ...
  updateCursor: () => {
    const totalTime = Math.max(1, State.simulation.totalTime || 1);
    const totalWidth = Math.max(UI.ganttWrapper.clientWidth || 800, Math.ceil(totalTime * 80) + 120);
    const pos = (State.playback.currentTime / totalTime) * totalWidth;
...
const Controller = {
  init: () => {
    ...
    // redraw on resize so long timelines remain visible and scaled
    window.addEventListener('resize', () => {
      // re-render gantt and cursor
      Renderer.drawGantt();
